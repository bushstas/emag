...
class BattleCreator {
	...
	static function create($options) {
		...
		$data = array(
			...
			'terrainId' => $area['terrainId'],
			'terrain' => self::generateTerrain($area)
		);
	...
	private function generateTerrain($options) {
		$terrainImageCount = 1;
		while (file_exists(__DIR__.'/../../assets/images/terrain/'.$options['terrainId'].'/'.$terrainImageCount.'.jpg')) {
			$terrainImageCount++;
		}
		$terrainImageCount--;
		$terrain = array();
		$width = self::$mapSize[0] + 4;
		$height = self::$mapSize[1] + 4;
		$prev = null;
		$idx = -1;
		for ($i = 0; $i < $height; $i++) {
			for ($j = 0; $j < $width; $j++) {
				$idx++;
				$cell = self::generateTerrainCell($terrainImageCount);
				if ($cell === $prev || (isset($terrain[$idx - $width]) && $cell === $terrain[$idx - $width])) {
					$j--;
					$idx--;
				} else {
					$terrain[] = $cell;
